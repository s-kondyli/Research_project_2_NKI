import pandas as pd
import sys
#this script takes as input: data to be filtered,excel with filters in sheets
#and gives as output an excel with the filtered data in the current working directory

#defining a function that takes data from excel and applies conditional filrering to columns based on a list of dictionaries
#the keys are the columns you apply a filter on and the values are threshold/filter
#2 dictionaries in the list: 1 for columns to be filtered for greater than/equal to a value & 2 for columns to be filtered for less than/equal to a value
...
print(filtered_data)
filtered_data.to_excel(excel_writer='filtered_data.xlsx', index=False)
